...
# === DIRECTORIES ===
STATES_DIR = pathlib.Path('../hadamard_ising_states_ED')
SAVE_DIR = pathlib.Path('../results_ising_ED_hadamard/fidelity_ED')
...
for i, _ in tqdm(enumerate(H_LIST[:-1])):
    state_name: str = f"N{N}_h{H_LIST[i]}_ising_{masks}.npy"
    state_name_2: str = f"N{N}_h{H_LIST[i+1]}_ising_{masks}.npy"
    psi: NDArray[np.complex128] = np.load(STATES_DIR / state_name)
    psi = psi / np.sqrt(sum(element ** 2 for element in psi))
